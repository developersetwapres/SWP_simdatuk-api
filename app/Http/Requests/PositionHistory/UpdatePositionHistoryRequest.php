...
class UpdatePositionHistoryRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'users.*.position' => 'required|max:160',
            'users.*.group_id' => 'numeric',
            'users.*.echelon_id' => 'numeric',
            'users.*.position_status' => 'numeric|in:1,2,3,4',
            'users.*.effective_date' => 'date',
            'users.*.decree' => 'max:160',
            ...
            'users.*.termination_date' => 'date',
            'users.*.termination_decree' => 'max:160',
            'users.*.type_of_termination_decree' => 'numeric',
            'users.*.termination_decree_number' => 'max:160',
            'users.*.termination_decree_date' => 'date',
    ...
    public function messages(): array
    {
        return [
            ...
            'users.*.position_id.required' => 'Jabatan tidak boleh kosong.',
            'users.*.position_id.max' => 'Jabatan tidak boleh lebih dari 160 karakter.',
            'users.*.group_id.numeric' => 'Rumpun harus berupa angka.',
            'users.*.echelon_id.numeric' => 'Eselon harus berupa angka.',
    ...
    public function bodyParameters(): array
    {
        return [
            ...
            'users.*.id' => [
                'description' => 'Refers to the ID of Employee Position.',
                'example' => 1,
            ],
            'users.*.user_id' => [
            ...
            'users.*.echelon_id' => [
                'description' => 'Refers to the ID Echelon of Employee Position.',
                'example' => 1,
            ],
            'users.*.position_status' => [
                'description' => 'Refers to the Position Status of Employee Position.',
                'example' => 1,
            ],
