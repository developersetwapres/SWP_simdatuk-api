...
class ExportRecapitulationController extends Controller
{
    ...
    private function getPositions($ids)
    {
        ...
        $positions->leftJoin('position_history_users as phu', 'u.id', '=', 'phu.user_id');
        ...
        foreach (json_decode(json_encode($positions), true) as $data) {
    ...
    private function getTrainings($ids, $type)
    {
        ...
        $trainings->leftJoin('training_history_users as thu', 'u.id', '=', 'thu.user_id');
        $trainings->leftJoin('training_histories as th', 'thu.training_history_id', '=', 'th.id');
        ...
        // Group the data based on IDs
        foreach (json_decode(json_encode($trainings), true) as $data) {
            if (in_array($data['id'], $ids)) {
                $groupedData[$data['id']][] = $data;
            }
        }
